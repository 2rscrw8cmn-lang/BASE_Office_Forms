...
export function createRevisionDetailView({
  ...
  function fileCard(file, primary = false) {
    return `<article class="document-file-card${primary ? " is-primary" : ""}"><div class="document-file-icon" aria-hidden="true"><svg class="app-icon" viewBox="0 0 24 24"><path d="M6 3.5h8l4 4V21H6z"></path><path d="M14 3.5V8h4M9 12h6M9 16h4"></path></svg></div><div class="document-file-copy"><strong>${escapeHtml(file.originalFilename)}</strong><span>${escapeHtml(fileTypeLabel(file.mediaType))} · ${escapeHtml(formatBytes(file.byteSize))} · Uploaded ${escapeHtml(formatDate(file.uploadedAt) || "—")}</span></div><a class="secondary-button" href="${fileHref(file.id)}" target="_blank" rel="noopener" aria-label="View ${escapeHtml(file.originalFilename)}">View file</a></article>`;
  }
  ...
  function loaded(data) {
    ...
    return `<nav class="document-breadcrumbs" aria-label="Breadcrumb"><a href="${documentsHref}" data-app-link>Documents</a><span aria-hidden="true">/</span><a href="${documentHref}" data-app-link>${escapeHtml(record.title)}</a><span aria-hidden="true">/</span><span aria-current="page">${escapeHtml(revisionName(revision))}</span></nav><header class="revision-identity"><div><p>${escapeHtml(record.recordNumber || "Unnumbered document")} · ${escapeHtml(recordTypeLabel(record.recordType))}</p><h2 id="page-title" tabindex="-1">${escapeHtml(revisionName(revision))}</h2><p>${escapeHtml(record.title)}</p></div><div class="revision-status-stack">${isArchived ? '<span class="status-badge status-neutral">Archived</span>' : ""}<span class="status-badge status-${isDraft ? "attention" : revision.status === "published" ? "success" : "neutral"}">${escapeHtml(revisionStatusLabel(revision.status))}</span>${revision.isCurrent ? '<span class="document-current-note">Current version</span>' : ""}</div></header>${isArchived ? `<div class="document-read-only"><strong>Archived document</strong><span>This version remains available but is read-only${record.archivedAt ? ` · Archived ${escapeHtml(formatDate(record.archivedAt))}` : ""}.</span></div>` : !isDraft ? '<div class="document-read-only"><strong>Read-only version</strong><span>Published and superseded versions cannot be changed from this workspace.</span></div>' : ""}<section class="revision-summary"><div><span>Change summary</span><p>${escapeHtml(revision.changeSummary || "No change summary provided.")}</p></div><div><span>Created</span><p>${escapeHtml(formatDate(revision.createdAt) || "—")}</p></div><div><span>Files</span><p>${revision.fileCount}</p></div><div><span>Issuance</span><p><span class="issuance-badge${revision.issuanceCount ? " is-issued" : ""}">${revision.issuanceCount ? `${revision.issuanceCount} issuance${revision.issuanceCount === 1 ? "" : "s"}` : "Not issued"}</span></p></div></section><section class="revision-document" aria-labelledby="revision-files-title"><div class="document-section-heading"><h3 id="revision-files-title">Version files</h3>${revision.files.length ? `<span>${revision.files.length} file${revision.files.length === 1 ? "" : "s"}</span>` : ""}</div>${
